...
st.subheader('Crypto Currency Price Prediction ')

csv = pd.read_csv('convertcsv.csv')
symbol = csv['symbol'].tolist()

# creating sidebar
ticker_input = st.selectbox('Choose Crypto Coin', symbol,index=symbol.index('ETH'))

start = dt.datetime.today() - dt.timedelta(3*365)
